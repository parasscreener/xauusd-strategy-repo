...
- Gold (XAU/USD) daily prices via **Alpha Vantage** `FX_DAILY` (from_symbol=`XAU`, to_symbol=`USD`).
- Broad US dollar index via **FRED** series `DTWEXBGS` (Nominal Broad U.S. Dollar Index, goods and services).
- 10Y real yield via **FRED** series `DFII10` (10-Year Treasury Inflation-Indexed Security).
...
2. In repository **Settings → Secrets and variables → Actions → New repository secret**, define:
   ...
   - `FRED_API_KEY` (your API key from St. Louis Fed FRED)
   - `ALPHAVANTAGE_API_KEY` (your API key from Alpha Vantage for FX_DAILY XAU/USD)
