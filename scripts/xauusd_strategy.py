...
def _select_dxy_close_column(df_dxy: pd.DataFrame) -> pd.Series:
    ...
    for cand in ["dxy_close", "close", "adj close", "adj_close"]:
        if cand in df_dxy.columns:
            s = df_dxy[cand].rename("dxy_close")
            return s
    raise KeyError("No suitable DXY close column found in df_dxy; available columns: " + ", ".join(df_dxy.columns))
