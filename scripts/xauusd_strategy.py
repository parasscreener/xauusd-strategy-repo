...
def fetch_gold_prices(lookback_years: int) -> pd.DataFrame:
    """Fetch XAU/USD daily via Alpha Vantage FX_DAILY (historical + current).
    ...
    if key not in data:
        raise RuntimeError(f"Unexpected Alpha Vantage response for XAU/USD: keys={list(data.keys())}")

    ts = data[key]
    if not isinstance(ts, dict) or not ts:
        raise RuntimeError("Alpha Vantage returned empty XAU/USD time series")
    ...
    # keep only needed columns, convert to float
    df = df[["open", "high", "low", "close"]].astype(float)

    start_cutoff = TODAY - dt.timedelta(days=365 * lookback_years + 30)
    df = df.loc[df.index >= pd.to_datetime(start_cutoff)]

    if df.empty:
        raise RuntimeError("No gold data after filtering by lookback; check Alpha Vantage response")

    return df


def fetch_dxy_index(lookback_years: int) -> pd.DataFrame:
    """Fetch broad USD index (DTWEXBGS) from FRED as DXY proxy.
    ...
    if not obs:
        raise RuntimeError("No observations returned from FRED for DTWEXBGS")

    df = pd.DataFrame(obs)
    df["date"] = pd.to_datetime(df["date"])
    df["dxy_close"] = pd.to_numeric(df["value"], errors="coerce")
...
def generate_daily_decision_table(df_gold: pd.DataFrame,
    ...
    if df_gold.empty:
        raise RuntimeError("No gold data downloaded; df_gold is empty. Check Alpha Vantage API or symbols.")
...
def run_daily_report(lookback_years: int, out_html: str, out_csv: str) -> None:
    ...
    dxy = fetch_dxy_index(lookback_years)
